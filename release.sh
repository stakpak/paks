...
get_current_version() {
    grep -A1 '\[workspace.package\]' Cargo.toml | grep 'version' | sed -E 's/version = "([^"]+)"/\1/'
}
...
# Main script logic
main() {
    print_info "Starting release process for Paks..."
    ...
    if [[ "$is_beta" == true ]]; then
        ...
        print_info "  curl -L https://github.com/stakpak/paks/releases/download/v${new_version}/paks-darwin-aarch64.tar.gz | tar xz"
        print_info "  sudo mv paks /usr/local/bin/"
    else
        print_success "Release $new_version completed successfully! 🎉"
        print_info "Homebrew formula will be updated automatically."
        print_info "Install via: brew install stakpak/stakpak/paks"
    fi
